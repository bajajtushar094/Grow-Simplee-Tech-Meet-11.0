  ...
  return (
    <div className="max-w-[224px] w-full p-6">
      ...
      {lhsOptions.map((options,index)=> {
          return (
          <div key={index} className={cx('w-full text-gs-text-gray font-semibold p-[14px] text-sm flex cursor-pointer',{'!text-gs-blue':(options.value.indexOf(location?.pathname?.split('/')[2])> -1)})} onClick={()=>{onTabClick(options)}}>
          <InventoryIcon  className={cx('mr-2',{'!stroke-gs-blue':(options.value.indexOf(location?.pathname?.split('/')[2]) > -1)})}/>
            {options.label}
          </div>)
        })}
      </div>
